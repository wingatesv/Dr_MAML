...
class Aux_MAML(MetaTemplate):
    def __init__(self, model_func,  n_way, n_support, approx = False, test_mode = False):
        ...
        self.inpainting_head = backbone.InpaintingHead(input_channels=64, output_channels=1)
    ...
    def set_forward(self,x, mask, is_feature = False):
        ...
        tissue_masks = mask_var[:,:self.n_support,:,:,:].contiguous().view( self.n_way* self.n_support, *mask.size()[2:]) #support data mask
    ...
    def set_forward_loss(self, x, mask):
        scores = self.set_forward(x, mask, is_feature = False)
    ...
    def train_loop(self, epoch, train_loader, optimizer): #overwrite parrent function
        ...
        #train
        for i, (x,mask,_) in enumerate(train_loader):
       
            self.n_query = x.size(1) - self.n_support
            assert self.n_way  ==  x.size(0), "Aux_MAML do not support way change"
            

            loss, scores = self.set_forward_loss(x, mask)
    ...
    def correct(self, x, mask):       
        scores = self.set_forward(x, mask)
    ...
    def test_loop(self, test_loader, return_std = False): #overwrite parrent function
        ...
        for i, (x,mask,_) in enumerate(tqdm(test_loader, desc='Testing', leave=False)):
            self.n_query = x.size(1) - self.n_support
            assert self.n_way  ==  x.size(0), "Aux_MAML do not support way change"
            correct_this, count_this, loss = self.correct(x, mask)
