...
class MAML(MetaTemplate):
    ...
    def set_forward(self,x, is_feature = False):
        ...
        if self.training_mode:
          ...
          x_a_i = torch.cat((x_a_i, x_a_i_clone), dim=0)
          y_a_i = torch.cat((y_a_i, y_a_i_clone), dim=0)
        ...
        for task_step in range(self.task_update_num): 
            ...
            fast_parameters = []
            affected_parameters = self.parameters() if self.current_epoch % 2 == 0 else self.classifier.parameters()
            
            for k, weight in enumerate(affected_parameters):
